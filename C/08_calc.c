...
int pop(LinkedStackType* s){
    ...
    else{ //노드가 존재하면 최상위 노드를 불러내어 값을 리턴하고 제거
...
void infix_to_postfix(char exp[], char str[]){
    ...
    init(&s); // 스택 초기화
    for (i = 0; i < len; i++){
        ch = exp[i]; // 입력받은 배열을 1간격씩 전진시키면서 문자를 확인하고
        switch (ch){ // 아래의 case문대로 조건 실행, 혹은 default에 따라 처리함
            case '+': case '-': case '*': case '/': // 연산자
                // 스택에 있는 연상자의 우선순위가 더 크거나 같으면 출력
...
// 후위표기식을 연산할 함수
int eval(char exp[]){
    int op1, op2, value, i = 0; // op1,2: 연산될 숫자를 pop한 변수, i: loop문 제어 변수
...
int main(){
    char* data; //입력받을 중위표현식이 저장될 char형 포이터변수
    char str[100] = { '\0' }; // 바뀐 후위표현식을 저장할 cahr형 배열
    int result; // 연산값이 저장될 변수
    int i = 0;
    data = (char*)calloc(100, sizeof(int)); // 배열의 크기 할당
    printf("후위표현식으로 표현할 중위 표현식을 입력하세요: ");
    gets(data);
